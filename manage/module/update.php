...
$unitprocess= $m_home.$icon."<a href=\"list.php\">".$m98.$mgw1."</a>".$icon.$mgw2.$m98; //單元路徑列
$ModuleNo = 98;
$Module_PKey = 98;
$Module_Name = $m98;
...
if ($_REQUEST["Submit"] == "送出"){
	...
	if ($_POST['intType']==1 && strLen($_POST['intUse']) == 0){
		...
		$rs = new recordset($sql,array(SqlFilter($_POST['intUse'],'int')));
		if(! $rs->eof){
	...
	if ($MSG == ""){
		...
		$pdo = new dbPDO();
		$table_name = 'module_p';
		$pdo->update($table_name,$data_array,'PKey',SqlFilter($Update_PKey,"int"));
		$SQL_U = $pdo->getLastSql()."\n".array_to_string($data_array).'PKey='.SqlFilter($Update_PKey,"int");
		unset($data_array);
		$pdo->close();	
	
		//寫入後台管理記錄
		manage_history($Module_PKey,$Module_Name,$SQL_U,$_SERVER['PHP_SELF'],$_SESSION["Login_ID"]);
		$Module_PKey = $Update_PKey;
		//產生子階層連結
		...
		//更新單元階層狀態
		if($_POST['oldLayer'] > $_POST['intLayer']){
			//$sql_d = "delete from module_d Where Module_PKey = ".SqlFilter($Module_PKey,"int")." and Sort > ".SqlFilter($_POST['intLayer'],'int');
			//execute_sql($sql_d);
			
			$pdo = new dbPDO();
			$table_name = 'module_d';
			$cond = ' Module_PKey = :Module_PKey and Sort > :Sort ';
			$data_array['Module_PKey'] = SqlFilter($Module_PKey,"int");
			$data_array['Sort'] = SqlFilter($_POST['intLayer'],'int');
			$pdo->delete($table_name,$cond,$data_array);
			unset($data_array);
			$pdo->close();
		}
		...
		if($_POST['oldColum'] > $_POST['intColum']){
			$pdo = new dbPDO();
			$table_name = 'module_c';
			$cond = ' Module_PKey = :Module_PKey and Sort > :Sort ';
			$data_array['Module_PKey'] = SqlFilter($Module_PKey,"int");
			$data_array['Sort'] = SqlFilter($_POST['intColum'],'int');
			$pdo->delete($table_name,$cond,$data_array);
			unset($data_array);
			$pdo->close();
			
			//$sql_d = "delete from module_c Where Module_PKey = ".SqlFilter($Module_PKey,"int")." and Sort > ".SqlFilter($_POST['intColum'],'int');
			//execute_sql($sql_d);
		}
		$show = "修改成功!";
