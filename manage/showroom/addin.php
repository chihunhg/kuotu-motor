...
if ($MSG == ""){
	...
	if (! $rs->eof){
		...
		$data_array['intType'] = $_POST['intType'];
		...
		$Showroom_PKey = $pdo->getLastId();
		$SQL_U = $pdo->getLastSql()."\n".array_to_string($data_array).'PKey='.$Showroom_PKey;
		unset($data_array);
		$pdo->close();
	...
	for ($i = 1; $i <= $total_uploads; $i++) {
		...
		if (strLen($Photo[$i]) > 10){			
			//判斷檔案類別(1.圖片;2.檔案)
			$Cond_Array['Showroom_PKey'] = $Showroom_PKey;
			$Cond_Array['Sort'] = $i;
			...
			$sql = 'Select * from '.$table_name.' Where Showroom_PKey= :Showroom_PKey and Sort= :Sort';
			...
			if (!$rs->eof){
				...
				$data_array['Showroom_PKey'] = SqlFilter($Showroom_PKey,"int");
