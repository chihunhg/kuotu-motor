...
if ($_REQUEST["motion"] == "del" AND is_numeric($_REQUEST["imgNo"])){
	...
	$Cond_Array['News_PKey'] = sqlFilter($_REQUEST["AD_PKey"],"int");
	$sql = 'Select * From news_img Where Sort= :Sort and AD_PKey= :AD_PKey';
	$rs = new recordset($sql,$Cond_Array);
	if (! $rs->eof){
		$PKey = authcode($rs->field("AD_PKey"),'ENCODE');
...
if (! $rs->eof){
	...
	$PhotoS = array();
	$PhotoM = array();
	$sql = 'Select * from news_img Where News_PKey= :News_PKey and Photo1 <> \'\' Order By Sort';
