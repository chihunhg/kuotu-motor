...
if ($MSG == ""){
	...
	if (! $rs->eof){
		...
		$News_PKey = $pdo->getLastId();
		$SQL_U = $pdo->getLastSql()."\n".array_to_string($data_array).'PKey='.$News_PKey;
		unset($data_array);
		$pdo->close();
	...
	for ($i = 1; $i <= $total_uploads; $i++) {
		...
		if (strLen($Photo[$i]) > 10){			
			//判斷檔案類別(1.圖片;2.檔案)
			$Cond_Array['News_PKey'] = $News_PKey;
			$Cond_Array['Sort'] = $i;
			...
			$sql = 'Select * from '.$table_name.' Where News_PKey= :News_PKey and Sort= :Sort';
			...
			if (!$rs->eof){
				...
				$data_array['News_PKey'] = SqlFilter($News_PKey,"int");
