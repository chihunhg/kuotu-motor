...
if($_REQUEST["Action"] == "del"){
	...
	$sql = 'Select * From news Where PKey IN ('.implode(",",$array_cond).')';
	$rs = new recordset($sql,$_POST["nid"]);
	while (! $rs->eof){
		$pdo = new dbPDO();
		$sql = 'Select * From news_img Where AD_PKey= :AD_PKey';
		...
		while (! $rs1->eof){
			...
			$pdo->delete('news_img',' PKey= :PKey',array($rs1->field("PKey")));
		$rs1->movenext();
		}// while End	
		
		$pdo = new dbPDO();
		$pdo->delete('news',' PKey= :PKey',array($rs->field("PKey")));
		$pdo->close();
	$rs->movenext();
	}// while End
	unset($array_cond);
...
$sql = "Select Count(PKey) as Total From news ".$PDO_Cond;
            ...
            <div class="table-container">
              <table width="100%" cellspacing="0" cellpadding="0" class="list">
				...
				while (! $rs->eof){
					...
					// $Photo1 = '';	
					// $sql = 'Select * From news_img Where News_PKey= :News_PKey and Photo1 <> \'\' Order By Sort limit 1';
					// $rs1 = new recordset($sql,array($rs->field("PKey")));
					// if (! $rs1->eof && is_file('../../Upload/'.$rs1->field("Forder").'/s_'.$rs1->field("Photo1"))){
					// 	$Photo1 = '../../Upload/'.$rs1->field("Forder").'/s_'.$rs1->field("Photo1");
					// }
					// $rs1->close();
					...
					if($rs->field("Home")=='Yes'){
						$Home = '<span class="orange"><i class="fa fa-check-circle"></i></span>';
					}
                ?>
