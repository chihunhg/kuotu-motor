...
if($_REQUEST["Action"] == "del"){
	...
	$sql = 'Select * From discount Where PKey IN ('.implode(",",$array_cond).')';
	$rs = new recordset($sql,$_POST["nid"]);
	while (! $rs->eof){
		$pdo = new dbPDO();
		$sql = 'Select * From discount_img Where Discount_PKey= :Discount_PKey';
		...
		while (! $rs1->eof){
			...
			$pdo->delete('discount_img',' PKey= :PKey',array($rs1->field("PKey")));
		$rs1->movenext();
		}// while End	
		
		$pdo = new dbPDO();
		$pdo->delete('discount',' PKey= :PKey',array($rs->field("PKey")));
		$pdo->close();
	$rs->movenext();
	}// while End
	unset($array_cond);
...
$sql = "Select Count(PKey) as Total From discount ".$PDO_Cond;
