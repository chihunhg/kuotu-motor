...
$(window).scroll(function(){ 
	...
	if (Math.ceil(wh+c)>=h){ 
		$('.scrolldown-rwd').addClass('bottom');
	} 
	else
	{
		$('.scrolldown-rwd').removeClass('bottom');
	}
})
...
$(document).ready(function(){
	$(window).scroll(function(){
		// if ($(this).scrollTop() > 0) {
			...
			$('.kuotu-bar-drop').slideUp();
			$('.dropdown-menu-mian').slideUp();
		// } else {
			// $('.index').removeClass('change');
		// }
	});
});

$(document).ready(function(){
	$(".kuotu-bar-ul-item").click(function(){
	  $(this).children('.kuotu-bar-drop').slideToggle(300);
	  $(this).siblings().children('.kuotu-bar-drop').slideUp();
	  $(this).toggleClass('open').siblings().removeClass('open');
	  $('.dropdown-menu-mian').slideUp();
	//   $(".nav-item-mian").removeClass('open');
	//   $('.dropdown-menu-mian').slideUp();
	});
...
$(document).mouseup(function (e) {
    var container =$(".nav-item"); // 這邊放你想要排除的區塊
    if (!container.is(e.target) && container.has(e.target).length === 0) {
       container.children('.kuotu-bar-drop').slideUp();
	   $('.nav-item-mian').children('.dropdown-menu-mian').slideUp();
       //$('#lan').removeClass('open');
    }
});

$(document).ready(function(){
	$(".nav-item-mian").click(function(){
	  $(this).children('.dropdown-menu-mian').slideToggle(300);
	  $(this).toggleClass('open').siblings().removeClass('open');
	  $(this).siblings().children('.dropdown-menu-mian').slideUp();
	  $(".nav-item").removeClass('open');
	  $('.kuotu-bar-drop').slideUp();
	});
});
...
function fbFunction() { 
	if (window.confirm("開啟外部連結")) {
		window.open("https://www.facebook.com/kuotusedcar/shop");
	}
}
function lexusFunction() { 
	if (window.confirm("開啟外部連結")) {
		window.open("https://www.facebook.com/LexusCpoKuotu/shop/?referral_code=page_shop_tab&preview=1");
	}
}  

function searchFunction() { 
	if (window.confirm("開啟外部連結")) {
		window.open("https://www.104.com.tw/company/aetgx1c");
	}
} 
